...
void writeMatrixFile(MyArray *matrix, string filePath, int time)
{
    ofstream file;
    file.open(filePath);
...
int main()
{
    ...
    if (matrixA->ncol == matrixB->nrow)
    {
        ...
        writeMatrixFile(matrixC, "../results/multiplicacaoProcesso.txt", chrono::duration_cast<chrono::milliseconds>(end - begin).count());
    }
    else
    {
